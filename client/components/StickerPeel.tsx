...
/* The peel effect duplicates and filters the same raw image in SVG layers. */
/* eslint-disable @next/next/no-img-element */

import { useRef, useEffect, useId, useMemo, CSSProperties } from 'react';
...
const StickerPeel: React.FC<StickerPeelProps> = ({
  ...
  const pointLightId = `point-light-${filterId}`;
  const pointLightFlippedId = `point-light-flipped-${filterId}`;
  const dropShadowId = `drop-shadow-${filterId}`;
  const expandAndFillId = `expand-fill-${filterId}`;
  ...
  useEffect(() => {
    ...
    const boundsEl = dragBoundsSelector
      ? document.querySelector<HTMLElement>(dragBoundsSelector)
